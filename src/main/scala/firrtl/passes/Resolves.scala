...
class ResolveKinds extends Pass with PreservesAll[Transform] {
  ...
  override def prerequisites: Set[Class[Transform]] =
    Set[Class[Transform]]( classOf[passes.CheckHighForm] ) ++ firrtl.stage.Forms.WorkingIR
...
class ResolveGenders extends Pass {
...
class CInferMDir extends Pass with PreservesAll[Transform] {
  ...
  override def prerequisites: Set[Class[Transform]] = firrtl.stage.Forms.ChirrtlForm ++
    Set[Class[Transform]]( classOf[CInferTypes] )
